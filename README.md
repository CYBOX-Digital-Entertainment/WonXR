...
3. 카메라 권한을 허용합니다.
4. 처음에는 위쪽/정면에서 교리도 전체가 화면에 들어오게 비춥니다.
5. 인식 후 `구역을 터치해 설명을 확인하세요`가 보이면 원하는 구역을 터치합니다.
6. 선택 구역의 raised 3D block과 하단 설명 카드가 나타나는지 확인합니다.
7. 3D 느낌은 20-35도 정도의 완만한 각도에서 확인하고, 너무 낮은 측면 각도는 피합니다.
